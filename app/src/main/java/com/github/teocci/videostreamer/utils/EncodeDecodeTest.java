package com.github.teocci.videostreamer.utils;
 ...
 * Created by teocci.
 *
 * @author teocci@yandex.com on 2017/Apr/07
 */

public class EncodeDecodeTest extends AndroidTestCase
{
    private static final String TAG = EncodeDecodeTest.class.getSimpleName();
    ...
    private static final int FRAME_RATE = 20;               // 20 fps
    ...
    private void setParameters(int width, int height, int bitRate) {
        if ((width % 16) != 0 || (height % 16) != 0) {
            LogHelper.e(TAG, "WARNING: width or height not multiple of 16");
        }
        mWidth = width;
        mHeight = height;
        mBitRate = bitRate;
    ...
    private void encodeDecodeVideoFromBuffer(boolean toSurface) throws Exception {
        ...
        try {
            ...
            if (codecInfo == null) {
                ...
                LogHelper.e(TAG, "Unable to find an appropriate codec for " + MIME_TYPE);
                return;
            }
            if (VERBOSE) LogHelper.e(TAG, "found codec: " + codecInfo.getName());
            int colorFormat = selectColorFormat(codecInfo, MIME_TYPE);
            if (VERBOSE) LogHelper.e(TAG, "found colorFormat: " + colorFormat);
            ...
            if (VERBOSE) LogHelper.e(TAG, "format: " + format);
            ...
            if (VERBOSE) LogHelper.e(TAG, "releasing codecs");
            if (encoder != null) {
                encoder.stop();
                encoder.release();
            }
            ...
            LogHelper.e(TAG, "Largest color delta: " + mLargestColorDelta);
        }
    }
    /**
     * Tests encoding and subsequently decoding video from frames generated into a buffer.
    ...
    private void doEncodeDecodeVideoFromBuffer(MediaCodec encoder, int encoderColorFormat,
        ...
        if (DEBUG_SAVE_FILE) {
            ...
            try {
                ...
                LogHelper.e(TAG, "encoded output will be saved as " + fileName);
            } catch (IOException ioe) {
                LogHelper.e(TAG, "Unable to create debug output file " + fileName);
        ...
        while (!outputDone) {
            if (VERBOSE) LogHelper.e(TAG, "loop");
            ...
            if (!inputDone) {
                ...
                if (VERBOSE) LogHelper.e(TAG, "inputBufIndex=" + inputBufIndex);
                if (inputBufIndex >= 0) {
                    ...
                    if (generateIndex == NUM_FRAMES) {
                        ...
                        inputDone = true;
                        if (VERBOSE) LogHelper.e(TAG, "sent input EOS (with zero-length frame)");
                    } else {
                        ...
                        if (VERBOSE) LogHelper.e(TAG, "submitted frame " + generateIndex + " to enc");
                    }
                    generateIndex++;
                } else {
                    // either all in use, or we timed out during initial setup
                    if (VERBOSE) LogHelper.e(TAG, "input buffer not available");
                }
            }
            ...
            if (!encoderDone) {
                ...
                if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    if (VERBOSE) LogHelper.e(TAG, "no output from encoder available");
                } else if (encoderStatus == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                    // not expected for an encoder
                    encoderOutputBuffers = encoder.getOutputBuffers();
                    if (VERBOSE) LogHelper.e(TAG, "encoder output buffers changed");
                } else if (encoderStatus == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                    // not expected for an encoder
                    MediaFormat newFormat = encoder.getOutputFormat();
                    if (VERBOSE) LogHelper.e(TAG, "encoder output format changed: " + newFormat);
                    ...
                    if (outputStream != null) {
                        ...
                        try {
                            ...
                            LogHelper.e(TAG, "failed writing debug data to file");
                            throw new RuntimeException(ioe);
                        }
                    }
                    if ((info.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) != 0) {
                        ...
                        if (VERBOSE) LogHelper.e(TAG, "decoder configured (" + info.size + " bytes)");
                    } else {
                        ...
                        if (VERBOSE) LogHelper.e(TAG, "passed " + info.size + " bytes to decoder"
                                + (encoderDone ? " (EOS)" : ""));
                    }
            ...
            if (decoderConfigured) {
                ...
                if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    // no output available yet
                    if (VERBOSE) LogHelper.e(TAG, "no output from decoder available");
                } else if (decoderStatus == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED) {
                    // The storage associated with the direct ByteBuffer may already be unmapped,
                    // so attempting to access data through the old output buffer array could
                    // lead to a native crash.
                    if (VERBOSE) LogHelper.e(TAG, "decoder output buffers changed");
                    decoderOutputBuffers = decoder.getOutputBuffers();
                } else if (decoderStatus == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                    // this happens before the first frame is returned
                    decoderOutputFormat = decoder.getOutputFormat();
                    if (VERBOSE) LogHelper.e(TAG, "decoder output format changed: " +
                            decoderOutputFormat);
                    ...
                    if (!toSurface) {
                        ...
                        rawSize += info.size;
                        if (info.size == 0) {
                            if (VERBOSE) LogHelper.e(TAG, "got empty frame");
                        } else {
                            if (VERBOSE) LogHelper.e(TAG, "decoded, checking frame " + checkIndex);
                        ...
                        if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                            if (VERBOSE) LogHelper.e(TAG, "output EOS");
                            outputDone = true;
                        }
                        decoder.releaseOutputBuffer(decoderStatus, false /*render*/);
                    } else {
                        if (VERBOSE) LogHelper.e(TAG, "surface decoder given buffer " + decoderStatus +
                                " (size=" + info.size + ")");
                        rawSize += info.size;
                        if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                            if (VERBOSE) LogHelper.e(TAG, "output EOS");
                        ...
                        if (doRender) {
                            if (VERBOSE) LogHelper.e(TAG, "awaiting frame " + checkIndex);
                            assertEquals("Wrong time stamp", computePresentationTime(checkIndex),
                                    info.presentationTimeUs);
        ...
        if (VERBOSE) LogHelper.e(TAG, "decoded " + checkIndex + " frames at "
                + mWidth + "x" + mHeight + ": raw=" + rawSize + ", enc=" + encodedSize);
        if (outputStream != null) {
            try {
                outputStream.close();
            } catch (IOException ioe) {
                LogHelper.e(TAG, "failed closing debug file");
                throw new RuntimeException(ioe);
            }
        }
        if (outputSurface != null) {
        ...
        while (!outputDone) {
            ...
            if (!inputDone) {
                if (generateIndex == NUM_FRAMES) {
                    ...
                    if (VERBOSE) LogHelper.e(TAG, "signaling input EOS");
                    ...
                    if (VERBOSE) LogHelper.e(TAG, "inputSurface swapBuffers");
            ...
            while (decoderOutputAvailable || encoderOutputAvailable) {
                ...
                if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                    ...
                    if (VERBOSE) LogHelper.e(TAG, "decoder output buffers changed (but we don't care)");
                } else if (decoderStatus == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                    // this happens before the first frame is returned
    ...
    private boolean checkFrame(int frameIndex, MediaFormat format, ByteBuffer frameData) {
        ...
        if (!isRecognizedFormat(colorFormat)) {
            LogHelper.e(TAG, "unable to check frame contents for colorFormat=" +
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (!isColorClose(testY, expY) ||
                ...
                LogHelper.e(TAG, "Bad frame " + frameIndex + " (rect=" + i + ": yuv=" + testY +
    ...
    private boolean checkSurfaceFrame(int frameIndex) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            //LogHelper.e(TAG, "GOT(" + frameIndex + "/" + i + "): r=" + r + " g=" + g + " b=" + b);
            int expR, expG, expB;
            if (i == frameIndex % 8) {
                // colored rect
                expR = TEST_R1;
            ...
            if (!isColorClose(r, expR) ||
                    !isColorClose(g, expG) ||
                    !isColorClose(b, expB)) {
                LogHelper.e(TAG, "Bad frame " + frameIndex + " (rect=" + i + ": rgb=" + r +
