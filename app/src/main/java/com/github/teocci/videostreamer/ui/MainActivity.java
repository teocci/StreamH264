package com.github.teocci.videostreamer.ui;
...
import com.github.teocci.videostreamer.AvcEncoder;
import com.github.teocci.videostreamer.R;
import com.github.teocci.videostreamer.utils.EncodeDecodeTest;
...
public class MainActivity extends Activity implements SurfaceHolder.Callback, PreviewCallback
{
    ...
    private int port;
    private ArrayList<byte[]> encDataList = new ArrayList<>();
    private ArrayList<Integer> encDataLengthList = new ArrayList<>();
            ...
            while (isStreaming) {
                ...
                if (empty) {
                    try {
                        ...
                        LogHelper.e(TAG, "InterruptedException in startCamera: " + e);
                        e.printStackTrace();
                    }
                    continue;
                }
                try {
                    ...
                    LogHelper.e(TAG, "IOException in startCamera: " + e);
                    e.printStackTrace();
                }
            }
        }
    ...
    public void onPreviewFrame(byte[] data, Camera camera)
    {
        ...
        if (this.isStreaming) {
            LogHelper.e(TAG, "Is streaming");
            if (this.encDataLengthList.size() > 100) {
                LogHelper.e(TAG, "OUT OF BUFFER");
                return;
            }
            ...
            if (encData.length > 0) {
                synchronized (this.encDataList) {
                    LogHelper.e(TAG, "showing the encoded data");
    ...
    {
        LogHelper.e(TAG, "startStream initiated");
        ...
        try {
            ...
            LogHelper.e(TAG, "SocketException in startCamera: " + e);
            e.printStackTrace();
            return;
        } catch (UnknownHostException e) {
            LogHelper.e(TAG, "UnknownHostException in startCamera: " + e);
            e.printStackTrace();
            return;
        }
        ...
        Thread thread = new Thread(senderRun);
        thread.start();
    ...
    private void startCamera()
    {
        ...
        try {
            ...
            LogHelper.e(TAG, "RuntimeException in startCamera: " + e);
            e.printStackTrace();
        }
    }
                ...
                new DialogInterface.OnClickListener()
                {
                    ...
                    public void onClick(DialogInterface dialog, int which)
                    {
                        ...
                        if (ip.length() > 0 && (port >= 0 && port <= 65535)) {
                            LogHelper.e(TAG, "Call to startStream");
                            startStream(ip, port);
                        } else {
                            LogHelper.e(TAG, "else in showStreamDlg: ");
                        }
                    }
                });
